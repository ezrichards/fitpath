...
const ExerciseNode = (props: { exercise: Exercise, index: number }) => {
    ...
    return (
        <>
            <Modal opened={opened} onClose={close} closeButtonProps={{
                size: 40
            }}>
                <h1 className="modalHeader">{props.exercise.name}</h1>
                <button className="modalButton" onClick={close}>Done</button>
            </Modal>

            <button style={{
                marginLeft: `${marginLeft}px`,
            }} className="node" id={props.exercise.name} onClick={open}>
